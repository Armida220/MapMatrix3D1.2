...
//******************************************************************************************
void CLVLPropItem0::OnDrawValue (CDC* pDC) //画列表值
{
	ASSERT_VALID (this);
	ASSERT_VALID (m_pWndList);
	...
	else if( this==m_pWndList->GetFilterItem() )
	{
		CRect rectFill = m_Rect;
		{
			CBrush br(RGB(255,255,160));//浅黄色
			rectFill.DeflateRect(0,1,0,1);//减小矩形宽度
			pDC->FillRect (rectFill,&br);//填充矩形区域
		}
	}
	else if (this==m_pWndList->GetCurLayerPropItem())
	{
		...
		{
			CBrush br(RGB(157,223,255));//浅蓝色
			rectFill.DeflateRect(0,1,0,1);
			pDC->FillRect (rectFill,&br);
		}
		else
		{
			CBrush br(RGB(107,203,205));//较深蓝色
			rectFill.DeflateRect(0,1,0,1);
			pDC->FillRect (rectFill,&br);	
		}
	}
	...
	rgnClipVal.CreateRectRgnIndirect (&rectValClip);//创建一矩形区域
	pDC->SelectClipRgn (&rgnClipVal);

	if (m_bIsInput)					//表示是输入过滤条件的条目
	{
		CBrush br(::GetSysColor(COLOR_INFOBK));//系统白色
		pDC->FillRect(m_RectName,&br);//白色填充名称矩形
	}
...
// 		}
		...
		CRgn rgnClipVal;	//一块矩形区域
		CRect rectValClip = rectValue;
		rectValClip.bottom = min (rectValClip.bottom, m_pWndList->m_rectList.bottom); //取最小值
...
//		if(!IsSelected() || m_bIsInput)
		{
			...
			if(col.bFrame)//是否有边框
			{
				CBrush bs(RGB(128,128,128));//浅灰色
				rc = rectValue;
				rc.left = rc.right-1;
				pDC->FrameRect(&rc,&bs);//填充边框颜色
...
//			pDC->FillRect(rt,&br);
		...
		case CLVLPropColumn0::COLOR:
			{
				CRect rectColor = rectValue; //颜色矩形
				rectColor.right = rectColor.left + rectColor.Height ();
				rectColor.DeflateRect (1, 1);

				CRect rectText = rectValue; //文字矩形
				rectText.left  = rectColor.right+1;

				COLORREF ct = (long)*m_PropValues[i];
				CBrush br (ct);
				pDC->FillRect (rectColor, &br);
				pDC->Draw3dRect (rectColor, 0, 0);//画一个三维矩形
...
//更新值状态 例如选中了 显示 则打上勾 在次选中 变成x 右键 变成 -  
//******************************************************************************************
...
void CLVLPropItem0::ShowBitmap(CDC * pDC,CRect rect, int offset)
{
	...
	CDC dcMemory;//声明内存dc
	dcMemory.CreateCompatibleDC(pDC);//创建内存兼容dc
	dcMemory.SelectObject(&(m_pWndList->m_bmProp));// 选入一张兼容位图
	...
	case CLVLPropColumn0::COLOR:
		{
			m_pWndList->EditItem (this,&col);
				...
				break;
	case CLVLPropColumn0::CHECK://选中更改状态
	case CLVLPropColumn0::RADIO://更改状态
		OnUpdateValue(&col);//更改col值
		break;
	default:;
	}
...
CLVLPropList0::CLVLPropList0()
{
	...
	m_bmProp.LoadBitmap(IDB_PROPERTY);//加载一张位图到位图对象
	m_bmProp.GetObject(sizeof(BITMAP),&m_bmPropInfo);//获取位图结构体信息 
...
//******************************************************************************************
void CLVLPropList0::OnDrawList (CDC* pDC)//画一个表单
{
	ASSERT_VALID (this);
	ASSERT_VALID (pDC);
...
// 			m_nStart = -1;
	...
	//((CTestVectorBar*)GetParent())->OnSelChange();
	GetParent()->SendMessage(WM_SEL_CHANGED);
}
//************************************************************************************
void CLVLPropList0::OnRButtonDown(UINT nFlags, CPoint point) //响应鼠标右键消息 变为全选
{
...
void CLVLPropList0::SetScrollSizes ()
{
	if (m_wndScrollHorz.GetSafeHwnd () != NULL)
	{
		...
		else
		{
			...
			m_nHorzScrollTotal = GetTotalColWid()/m_nColWidth;
