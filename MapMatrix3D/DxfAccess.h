...
public:
	...
	void ImportLidarDxf();
	void ImportDwg_New(CString dwgPath);
protected:
	void ReadBlocks_dxf();
...
protected:
	...
	CComBSTR GetSubSymbolItemType(SubSymbolItem *pCurItem = NULL);

	//这几个函数只保存地物，可能会引用到符号名，但不生成CAD符号
	void SavePoint(CGeometry *pGeoObj, LPCTSTR dxfLayerName, LPCTSTR blkName = NULL, SubSymbolItem *pCurItem = NULL);
	void SaveCurveOrSurface(CGeometry *pGeoObj, LPCTSTR dxfLayerName, LPCTSTR linetype_name = NULL, float wid = -1, float thickness = -1, SubSymbolItem *pCurItem = NULL);
	void SaveText(CGeoText *pGeoObj, LPCTSTR dxfLayerName, SubSymbolItem *pCurItem = NULL);
	void SaveText_core(CGeoText *pGeoObj, LPCTSTR dxfLayerName, SubSymbolItem *pCurItem = NULL);  //SaveText是入口函数， SaveText调用 SaveText_core
	void SaveHatch(BOOL bIsBlock, CGeometry *pGeoObj, LPCTSTR dxfLayerName, LPCTSTR linetype_name = NULL, float wid = -1, float thickness = -1, SubSymbolItem *pCurItem = NULL);
	void SaveDCurveOrParallel(CGeometry *pGeoObj, LPCTSTR dxfLayerName, LPCTSTR linetype_name = NULL, float wid = -1, float thickness = -1, SubSymbolItem *pCurItem = NULL);

	//生成块，如果块已经存在，就插入其引用点
	void SetBlock(CFeature *pFtr, LPCTSTR dxfLayerName, CSymbolArray& arrPSyms, BOOL bNewBlock, LPCTSTR blkName = NULL, CFtrLayer *pLayer = NULL, SubSymbolItem *pCurItem = NULL);
	
	//不生成块，生成散的地物时，使用下面两个函数
	void SetBlock_exploded(CFeature *pFtr, LPCTSTR dxfLayerName, CSymbolArray& arrPSyms, CFtrLayer *pLayer = NULL, SubSymbolItem *pCurItem = NULL);
