...
HTREEITEM CTreeCtrlEx::InsertRoot(LPCTSTR lpszItem, int type, DWORD_PTR data, HTREEITEM hInsertAfter)
{
	HTREEITEM hRoot = CTreeCtrl::InsertItem (lpszItem, TVI_ROOT, hInsertAfter);
	if( hRoot )
	{
		SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);
		SetItemData(hRoot, data);
		SetItemImage(hRoot, ImageIndexOfType(type), ImageIndexOfType(type));
	}
	
	return hRoot;
}

HTREEITEM CTreeCtrlEx::InsertSubItem(LPCTSTR lpszItem, HTREEITEM hParent, int type, DWORD_PTR data, HTREEITEM hInsertAfter)
{ 
	HTREEITEM hSubRoot = CTreeCtrl::InsertItem (lpszItem, hParent, hInsertAfter);
	if( hSubRoot )
	{
		
		SetItemData(hSubRoot, data);
		SetItemImage(hSubRoot, ImageIndexOfType(type), ImageIndexOfType(type));
	}
	
	return hSubRoot;
}

HTREEITEM CTreeCtrlEx::FindItemByData(DWORD_PTR dwData, HTREEITEM hParent)
{
	if( hParent==NULL )
		hParent = GetRootItem();
	if( hParent==NULL )
	...
	while( hParent )
	{
		hItem = FindItemByData(dwData,hParent);
		if( hItem!=NULL )
			return hItem;
...
void CTreeCtrlEx::GetSelectedList(CTreeItemList& list) const
{
	list.RemoveAll();
...
int CTreeCtrlEx::ImageIndexOfType(int type)
{
	return -1;
}
/////////////////////////////////////////////////////////////////////////////
...
void CTreeCtrlEx::OnLButtonDown( UINT nFlags, CPoint point )
{
	...
	m_bLButtonDown = TRUE; //hcw,2012.3.23
	UINT nHitFlags = 0;
	...
	{
		//ClearSelection();//hcw,2012.3.23,clear all selection
...
void CTreeCtrlEx::OnLButtonUp( UINT nFlags, CPoint point )
{
	...
	m_hClickedItem=NULL;
	CTreeCtrl::SetFocus();//hcw,2012.3.23,regain the lost focus
...
void CTreeCtrlEx::SelectMultiple( HTREEITEM hClickedItem, UINT nFlags, CPoint point )
{
	...
	if ( nFlags & MK_SHIFT )
	{
		...
		m_bRealMulti = TRUE;//hcw,2012.3.23,multiSelected;
	...
	else
	{
		...
		m_bRealMulti = FALSE ;//hcw,2012.3.23,single selected;
		SelectItem( hClickedItem );
		SetItemState( hClickedItem, TVIS_SELECTED, TVIS_SELECTED );
...
///////////////////////////////////////////////////////////////////////////////
//hcw,2012.3.23 Clear selected state on all visible items
...
//hcw,2012.3.23,time interval for editing treeNode Node?¡ê
...
HTREEITEM CTreeCtrlEx::ItemFromData(DWORD_PTR dwData, HTREEITEM hStartAtItem/*=NULL*/) const
{
	...
	while ( hItem )
	{
		if (dwData == (DWORD_PTR)GetItemData(hItem))
			return hItem;
