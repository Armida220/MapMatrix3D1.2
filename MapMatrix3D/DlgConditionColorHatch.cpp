// DlgConditionColorHatch.cpp : implementation file
//
...
CListCtrlEx::EditorInfo::EditorInfo():m_pfnInitEditor(NULL)
,m_pfnEndEditor(NULL)
,m_pWnd(NULL)
,m_bReadOnly(false)
{
}
CListCtrlEx::EditorInfo::EditorInfo(PFNEDITORCALLBACK pfnInitEditor, PFNEDITORCALLBACK pfnEndEditor, CWnd *pWnd):m_pfnInitEditor(pfnInitEditor)
...
void CListCtrlEx::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CellInfo *pCellInfo = GetCellInfo(nItem,0);
	if(pCellInfo)
	{
		CRect rcDraw = rcCol;
		rcDraw.DeflateRect(0,1);
		pDC->FillRect(rcDraw, &CBrush(pCellInfo->m_clrBack));
	}
    ...
    for(int nColumn = 1; GetColumn(nColumn, &lvc); nColumn++)
    {
		...
		CellInfo *pCellInfo = GetCellInfo(nItem,nColumn);
		if(pCellInfo)
		{
			CRect rcDraw = rcCol;
			rcDraw.DeflateRect(0,1);
...
static BOOL EndEditor(CWnd** pWnd, int nRow, int nColumn, CString& strSubItemText, DWORD_PTR dwItemData, void* pThis, BOOL bUpdate)
{
	...
	(*pWnd)->GetWindowText(strSubItemText);

	if(pThis!=0)
	{
		CListCtrlEx *p = (CListCtrlEx*)pThis;
		DWORD_PTR data2 = p->GetItemData(nRow);
		
		if(data2!=0)
			((CDlgConditionColorHatch*)data2)->OnEndEdit(nRow,nColumn);
	}
...
/////////////////////////////////////////////////////////////////////////////
// CDlgConditionColorHatch dialog


CDlgConditionColorHatch::CDlgConditionColorHatch(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgConditionColorHatch::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlgConditionColorHatch)
...
void CDlgConditionColorHatch::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgConditionColorHatch)
	DDX_Control(pDX, IDC_COLOR_EDIT, m_wndColor);
	DDX_Control(pDX, IDC_COMBO_FIELDNAME, m_wndComboFields);
	DDX_Control(pDX, IDC_LIST_CONDITION, m_wndListConditions);
	DDX_Text_Silence(pDX, IDC_TRANSPARENCE_EDIT, m_fTransparence);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CDlgConditionColorHatch, CDialog)
	//{{AFX_MSG_MAP(CDlgConditionColorHatch)
	ON_BN_CLICKED(IDC_BUTTON_ADD, OnButtonAdd)
	ON_BN_CLICKED(IDC_BUTTON_DEL, OnButtonDel)
	ON_BN_CLICKED(IDC_COLOR_EDIT, OnDefColor)
	ON_BN_CLICKED(IDC_BUTTON_IMPORTTXT, OnImportTxt)
	ON_EN_CHANGE(IDC_TRANSPARENCE_EDIT, OnChangeTransparenceEdit)
	ON_NOTIFY(NM_CLICK, IDC_LIST_CONDITION, OnClickListCondition)
	ON_CBN_SELCHANGE(IDC_COMBO_FIELDNAME, OnSelchangeComboFieldname)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDlgConditionColorHatch message handlers


void CDlgConditionColorHatch::SetMem(CConditionColorHatch *pSym, CValueTable &tab)
{
	m_pSymInput = pSym;
	m_sym.CopyFrom(pSym);	

	m_ValTab.CopyFrom(tab);	
}


void CDlgConditionColorHatch::SaveSymbol()
{
	m_pSymInput->CopyFrom(&m_sym);

	CWnd *pWnd = GetParentOwner();
	pWnd->SendMessage(WM_SYMBOL_REPAINT);
}

BOOL CDlgConditionColorHatch::OnInitDialog() 
{
	...
	CVariantEx *var;
	CString strSelFiled = "";
	for (int i=0;i<m_ValTab.GetFieldCount();i++)
	{		
		m_ValTab.GetField(i,field,type,name);

		m_wndComboFields.AddString(name);
	}

	m_wndComboFields.SelectString(0,m_sym.m_strFieldName);

	m_wndColor.SetColor(m_sym.m_nDefColor);

	m_fTransparence = m_sym.m_fDefTransparence;

	//m_wndListConditions.SetExtendedStyle(m_wndListConditions.GetExtendedStyle() | LVS_EX_GRIDLINES);

	m_wndListConditions.InsertColumn(0,StrFromResID(IDS_FIELD_VALUE),LVCFMT_LEFT,85);
	m_wndListConditions.InsertColumn(1,StrFromResID(IDS_COLOR),LVCFMT_LEFT,85);
	m_wndListConditions.InsertColumn(2,StrFromResID(IDS_TRANSPARENT),LVCFMT_LEFT,85);

	for(i=0; i<m_sym.m_Conditions.GetSize(); i++)
	{
		CConditionColorHatch::Condition item = m_sym.m_Conditions[i];
		m_wndListConditions.InsertItem(i,"",0);
		m_wndListConditions.SetItemText(i,0,item.value);
		CString strText;
		strText.Format("%.2f",(double)item.fTransparence);
		m_wndListConditions.SetItemText(i,2,strText);
		m_wndListConditions.SetCellColors(i,1,item.color,0);
		m_wndListConditions.SetItemData(i,(DWORD_PTR)this);
	}

	m_wndListConditions.SetColumnReadOnly(0,false);
	m_wndListConditions.SetColumnReadOnly(2,false);
	m_wndListConditions.SetColumnReadOnly(1,true);

	m_wndEditForList.Create(ES_LEFT|ES_AUTOHSCROLL,CRect(0,0,0,0),this,101);

	m_wndListConditions.SetDefaultEditor(InitEditor,EndEditor,&m_wndEditForList);
...
void CDlgConditionColorHatch::OnButtonAdd() 
{
	CConditionColorHatch::Condition item;
	memset(&item,0,sizeof(item));

	strcpy(item.value,"...");
	item.color = m_wndColor.GetColor();
	item.fTransparence = m_fTransparence;
	
	int index = m_wndListConditions.GetItemCount();
	m_wndListConditions.InsertItem(index,"",0);
	m_wndListConditions.SetItemText(index,0,item.value);

	CString strText;
	strText.Format("%.2f",(double)item.fTransparence);
	m_wndListConditions.SetItemText(index,2,strText);

	m_wndListConditions.SetCellColors(index,1,item.color,0);
	m_wndListConditions.SetItemData(index,(DWORD_PTR)this);

	m_sym.m_Conditions.Add(item);

	SaveSymbol();
}

void CDlgConditionColorHatch::OnButtonDel() 
{
	for(int i=m_wndListConditions.GetItemCount()-1; i>=0; i--)
	{
		if(m_wndListConditions.GetItemState(i,LVIS_SELECTED)==LVIS_SELECTED )
		{
			m_wndListConditions.DeleteItem(i);
			m_sym.m_Conditions.RemoveAt(i);
		}
	}
	SaveSymbol();
}

void CDlgConditionColorHatch::OnClickListCondition(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_LISTVIEW* pNMListView = (NM_LISTVIEW*)pNMHDR;
	...
	*pResult = 0;
	...
	if( pNMListView->iSubItem==1 && pNMListView->iItem>=0 && pNMListView->iItem<m_sym.m_Conditions.GetSize() )
	{
		CColorDialog dlg;
		if( dlg.DoModal()!=IDOK )
			return;

		m_wndListConditions.SetCellColors(pNMListView->iItem,pNMListView->iSubItem,dlg.GetColor(),0);
		m_wndListConditions.RedrawWindow();

		m_sym.m_Conditions[pNMListView->iItem].color = dlg.GetColor();
		SaveSymbol();
	}
}


void CDlgConditionColorHatch::OnEndEdit(int nRow, int nCol)
{
	CString strText;
	m_wndEditForList.GetWindowText(strText);

	if(nRow>=0 && nRow<m_sym.m_Conditions.GetSize())
	{
		CConditionColorHatch::Condition item = m_sym.m_Conditions[nRow];
		if(nCol==0)
		{
			strncpy(item.value,strText,sizeof(item.value)-1);
		}
		else if(nCol==2)
		{
			item.fTransparence = atof(strText);
		}

		m_sym.m_Conditions[nRow] = item;
		
		int index = nRow;
		m_wndListConditions.SetItemText(index,0,item.value);

		CString strText;
		strText.Format("%d",(int)(item.fTransparence*100));
		m_wndListConditions.SetItemText(index,2,strText);
...
void CDlgConditionColorHatch::OnOK()
{
	
}


void CDlgConditionColorHatch::OnCancel()
{
	
}

void CDlgConditionColorHatch::OnSelchangeComboFieldname() 
{
	int nSel = m_wndComboFields.GetCurSel();
	if(nSel<0)
		return;
	
	m_wndComboFields.GetLBText(nSel,m_sym.m_strFieldName);	
	SaveSymbol();
}


void CDlgConditionColorHatch::OnDefColor()
{
	m_sym.m_nDefColor = m_wndColor.GetColor();
	SaveSymbol();
}


void CDlgConditionColorHatch::OnChangeTransparenceEdit() 
{
	UpdateData(TRUE);
	m_sym.m_fDefTransparence = m_fTransparence;
	SaveSymbol();	
}


extern void RemoveChar(char *str, const char *c);
void CDlgConditionColorHatch::OnImportTxt() 
{
	CFileDialogEx dlg(StrFromResID(IDS_SELECT_FILE), TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, StrFromResID(IDS_LOADTEXT_FILTER), this);
	if(dlg.DoModal()!=IDOK)
		return;
	...
	while(!feof(fp))
	{
		...
		if( strlen(line)>0 )
		{
			...
			item.color = RGB(rand()%105 + 150,rand()%105 + 150,rand()%105 + 150);
			item.fTransparence = m_fTransparence;
