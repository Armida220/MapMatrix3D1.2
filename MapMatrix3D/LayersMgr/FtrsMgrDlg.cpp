// FtrsMgrDlg.cpp : implementation file
//
...
//BOOL CFtrsMgrDlg::m_bWarningAll = FALSE;
...
CFtrsMgrDlg::CFtrsMgrDlg(CWnd* pParent /*=NULL*/)
: CManageBaseDlg(CFtrsMgrDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CFtrsMgrDlg)
...
void CFtrsMgrDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CFtrsMgrDlg)
	// NOTE: the ClassWizard will add DDX and DDV calls here
	DDX_Control(pDX,IDC_STATIC_STATUS,m_wndStatus);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CFtrsMgrDlg, CDialog)
	//{{AFX_MSG_MAP(CFtrsMgrDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CFtrsMgrDlg message handlers

BOOL CFtrsMgrDlg::OnInitDialog()
{
	m_pDlgDoc = GetActiveDlgDoc();
	if( !m_pDlgDoc )
	{
	...
	CreateToolbar(m_wndToolBarFilter,IDR_TOOLBARFTRMANAGE);
...
void CFtrsMgrDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CFtrsMgrDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}

BOOL CFtrsMgrDlg::CreateToolbar(CMFCToolBar& toolbar, UINT ID)
{
...
void CFtrsMgrDlg::OnSize(UINT nType, int cx, int cy) 
{
	CDialog::OnSize(nType, cx, cy);
	AdjustLayout();	
}

void CFtrsMgrDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
...
//表示鼠标的光标变为用作切分窗口的光标
BOOL CFtrsMgrDlg::MouseCursorIsSplitter(CPoint pt)
{
	RECT	rtTreeCtrl;
	RECT	rtListCtrl;
	m_wndTreeCtrl.GetWindowRect(&rtTreeCtrl);
...
void CFtrsMgrDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CFtrsMgrDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
...
BOOL CFtrsMgrDlg::OnEraseBkgnd(CDC* pDC) 
{
...
// 	CLVLPropItem *pItem;
// 	if(m_wndPropListCtrl.GetItemCount()>0)pItem = m_wndPropListCtrl.GetPropItem(m_wndPropListCtrl.GetItemCount()-1);
// 	m_wndPropListCtrl.SelectItem(pItem);
}

void CFtrsMgrDlg::OnSelectAllLayersExceptCurSels()
{
...
void CFtrsMgrDlg::OnUpdateSelectAllLayersExceptCurSels(CCmdUI *pCmdUI)
{
	DWORD_PTR dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::OnUpdateSelectAllLayers(CCmdUI *pCmdUI)
{
	DWORD_PTR dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::OnUpdateDeSelectLayers(CCmdUI *pCmdUI)
{
...
void CFtrsMgrDlg::OnUpdateDelLayerObj(CCmdUI *pCmdUI)
{
	DWORD_PTR dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::OnUpdateActiveLayer(CCmdUI *pCmdUI)
{
	DWORD_PTR dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::OnUpdateDeleteFilter(CCmdUI *pCmdUI)
{
	DWORD_PTR dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::OnUpdateRenameFilter(CCmdUI *pCmdUI)
{
	DWORD_PTR dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::OnUpdateNewLayer(CCmdUI *pCmdUI)
{
	DWORD_PTR dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::OnUpdateDelLayer(CCmdUI *pCmdUI)
{
	DWORD_PTR dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
// 		CFtrLayer *pLayer = pDS->GetFtrLayer(strLayerName);
		...
		if( pLayer->GetObjectCount()>0 )continue;
...
void CFtrsMgrDlg::OnInitMenuPopup(CMenu *pPopupMenu, UINT nIndex,BOOL bSysMenu)
{	
...
void CFtrsMgrDlg::OnDelLayerObj()
{
...
void CFtrsMgrDlg::OnActiveLayer()
{
	if (m_wndPropListCtrl.GetSelectedCount()<=0)
	{
		return;
...
//全选
void CFtrsMgrDlg::OnSelectAllLayers()
{
	m_wndPropListCtrl.SelectAll();
}

//取消选择的层
void CFtrsMgrDlg::OnDeSelectLayers()
{
	m_wndPropListCtrl.DeselectAll();
}

void CFtrsMgrDlg::OnCutSelectLayers()
{
	int nCount = m_wndPropListCtrl.GetSelectedCount();
	for (int i=0;i<nCount;i++)
	{
...
void CFtrsMgrDlg::OnUpdateCutSelectLayers(CCmdUI *pCmdUI)
{
	DWORD_PTR  dw;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
	BOOL flag = FALSE;
...
void CFtrsMgrDlg::OnPasteLayer()
{
	...
	{			
		FillFilterLayersList(m_wndTreeCtrl.GetItemText(hItem),m_pDlgDoc->GetDlgDataSource());
	}
...
void CFtrsMgrDlg::OnUpdatePasteLayer(CCmdUI *pCmdUI)
{
	DWORD_PTR  dw;
	BOOL flag = FALSE;
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::OnPasteLayerEx()
{
	if (!m_bIsCuttingObj||m_wndPropListCtrl.GetSelectedCount()!=1)return;
	CFtrLayer *pLayer = (CFtrLayer *)m_wndPropListCtrl.GetSelectedItem(0)->GetData();
	long lSum = 0;	
...
void CFtrsMgrDlg::OnUpdatePasteLayerEx(CCmdUI *pCmdUI)
{
	if (m_wndPropListCtrl.GetSelectedCount()==1)
	{
...
void CFtrsMgrDlg::OnResetDisplayOrder()
{
	CDlgDataSource *pDS = m_pDlgDoc->GetDlgDataSource();
...
void CFtrsMgrDlg::OnResetDefalutLayerColor()
{
	CDlgDataSource *pDS = m_pDlgDoc->GetDlgDataSource();
...
void CFtrsMgrDlg::OnExit()
{
	OnCancel();
}

void CFtrsMgrDlg::SaveLayers()
{
...
//保存过滤器
void CFtrsMgrDlg::SaveFilter(LPCTSTR name)
{
	CString strName;
	if( name )strName = name;		//获取过滤器名
	else
	{
		HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
// 			m_wndTreeCtrl.Expand(hFilter,TVE_EXPAND);			
			m_wndTreeCtrl.SelectItem(hFeatures);
		}
		else
		{
...
void CFtrsMgrDlg::LoadFilterFile(const char *szFileName)
{
	RemoveFilter(NULL);
...
// 		}
		...
		FilterItem *pFilter = FindFilterItem(_T("All Features"));
		if( !pFilter )
		{
			FilterItem *pFilter = new FilterItem;
			if( !pFilter ) return;
...
void CFtrsMgrDlg::SaveFilterFile(const char *szFileName)
{
	CMarkup xmlfile;
...
void CFtrsMgrDlg::RemoveFilter(LPCTSTR name)
{
	int nsz = m_arrFilters.GetSize();
...
void CFtrsMgrDlg::FillAllLayersList(CDlgDataSource *pDS,BOOL bForLoad )
{
	CValueTable tab;
	CFtrLayer *pLayer = new CFtrLayer;
	if( !pLayer )return;
...
// 		str.LoadString(IDS_FIELDNAME_DEFCOLOR);
		...
		else
		{
			if( col.ValueType==VT_BOOL )
			{
				...
				if (strcmp(col.FieldName,FIELDNAME_LAYINHERENT)==0)
				{
					col.bReadOnly = TRUE;
				}
	...
	for( i=0; i<nLayer; i++)
	{
		...
		for( int j=0, k=0; j<nCol; j++)
		{
			if(j==1)
			{
				...
				pItem->SetValue((_variant_t)(LPCTSTR)strcode,j);
...
*/
void CFtrsMgrDlg::FillAllFeatures(LPCTSTR filter, CDlgDataSource *pDS)
{
	...
	for(int i=0; i<nLayer; i++)
	{
		...
		CString dbLayerName = (schemelayer==NULL?CString():schemelayer->GetDBLayerName());
		...
		for (int j=0; j<pLayer->GetObjectCount(); j++)
		{
			...
			int geotype = pFtr->GetGeometry()->GetClassType();
			...
			for (int k=0; k<nFtrItemSize; k++)
			{
				if (stricmp(m_arrFtrItems[k].pFtrLayer->GetName(),pLayer->GetName()) == 0 && 
						 ...
						 m_arrFtrItems[k].code.CompareNoCase(pFtr->GetCode()) == 0 && m_arrFtrItems[k].dbLayerName.CompareNoCase(dbLayerName)==0 &&
						 m_arrFtrItems[k].geotype==geotype)
				{
					// 扩展属性
					BOOL bSame = TRUE;
			...
			item.dbLayerName = (schemelayer==NULL?CString():schemelayer->GetDBLayerName());
			item.ftrs.Add(pFtr);
	...
	//col.BitmapOffset = 1;
	col.bReadOnly = TRUE;
	col.arrComboValues.Add(StrFromResID(IDS_GEO_POINT));
	col.arrComboValues.Add(StrFromResID(IDS_GEO_CURVE));
	col.arrComboValues.Add(StrFromResID(IDS_GEO_SURFACE));
	col.arrComboValues.Add(StrFromResID(IDS_GEO_TEXT));
	col.arrComboValues.Add(_T(""));
	m_wndPropListCtrl.AddColumn(col);
...
// 		}
		...
		pItem->SetValue((_variant_t)(LPCTSTR)m_arrFtrItems[i].dbLayerName,6);	

		CString str_geotype;
		switch(m_arrFtrItems[i].geotype)
		{
		case CLS_GEOPOINT:
			str_geotype = StrFromResID(IDS_GEO_POINT);
			break;
		case CLS_GEODIRPOINT:
			str_geotype = StrFromResID(IDS_GEO_DIRPOINT);
			break;
		case CLS_GEOSURFACEPOINT:
			str_geotype = StrFromResID(IDS_GEO_SURFACEPOINT);
			break;
		case CLS_GEOMULTIPOINT:
			str_geotype = StrFromResID(IDS_GEO_MULTIPOINT);
			break;
		case CLS_GEODEMPOINT:
			str_geotype = StrFromResID(IDS_GEO_DEMPOINT);
			break;
		case CLS_GEOCURVE:
			str_geotype = StrFromResID(IDS_GEO_CURVE);
			break;
		case CLS_GEODCURVE:
			str_geotype = StrFromResID(IDS_GEO_DCURVE);
			break;
		case CLS_GEOPARALLEL:
			str_geotype = StrFromResID(IDS_GEO_PARALLEL);
			break;
		case CLS_GEOMULTISURFACE:
			str_geotype = StrFromResID(IDS_GEO_MULTISURFACE);
			break;
		case CLS_GEOSURFACE:
			str_geotype = StrFromResID(IDS_GEO_SURFACE);
			break;
		case CLS_GEOTEXT:
			str_geotype = StrFromResID(IDS_GEO_TEXT);
			break;
		default:
			str_geotype = StrFromResID(IDS_GEO_NONE);
			break;
		}
		pItem->SetValue((_variant_t)(LPCTSTR)str_geotype,7);
		...
		for (int j=0; j<xSize; j++)
		{
			...
			{
				pItem->SetValue((_variant_t)(LPCTSTR)(xItem.value),8+j);
				pItem->SetColReadOnly(8+j,TRUE);
			}
			else if (xItem.valuetype == DP_CFT_BOOL)
			{
				pItem->SetValue((_variant_t)(bool)atol(xItem.value),8+j);
			}
// 			else if (xItem.valuetype == DP_CFT_INTEGER)
// 			{
// 				pItem->SetValue((_variant_t)atol(xItem.value),8+j);
// 			}
			else
			{
				pItem->SetValue((_variant_t)(LPCTSTR)(xItem.value),8+j);
		...
		pItem->SetValue((_variant_t)(long)m_arrFtrItems[i].ftrs.GetSize(),8+xSize);
...
void CFtrsMgrDlg::FillAllGroups(CDlgDataSource *pDS)
{
/*	m_arrLayerGroups.RemoveAll();
	...
	CLVLPropItem *pItem;
	for (i=0; i<m_arrLayerGroups.GetSize(); i++)
	{
		pItem = new CLVLPropItem;
		if( !pItem )continue;
		...
		pItem->SetValue((_variant_t)(LPCTSTR)m_arrLayerGroups[i].name,0);
		pItem->SetValue((_variant_t)m_arrLayerGroups[i].color,1);
		pItem->SetValue((_variant_t)(bool)m_arrLayerGroups[i].bVisible,2);
		pItem->SetValue((_variant_t)(bool)m_arrLayerGroups[i].bSymbolized,3);
		pItem->SetData((DWORD_PTR)(m_arrLayerGroups.GetData()+i));
...
void CFtrsMgrDlg::FillFilterLayersList(LPCTSTR filter, CDlgDataSource *pDS, BOOL bOnlyUsedLayers)
{
	FilterItem *pFilter = FindFilterItem(filter);
	if( !pFilter )return;
...
// 		var = bool(true);
// 	//	if()
// 		pItem->tab.AddValue("LAYERUSED",&CVariantEx(var));
// 	}
// 	pItem->tab.EndAddValueItem();
	m_arrFilters.Add(pItem);
}

FilterItem* CFtrsMgrDlg::FindFilterItem(LPCTSTR name)
{
...
void CFtrsMgrDlg::OnRenameFilter() 
{
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
	if( hItem==NULL || (m_wndTreeCtrl.GetItemData(hItem)!=LM_TREEID_FILTERITEM&&m_wndTreeCtrl.GetItemData(hItem)!=LM_TREEID_FILTERITEM_REF) )
		return;
...
void CFtrsMgrDlg::OnDeleteFilter() 
{
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
	if( hItem==NULL || (m_wndTreeCtrl.GetItemData(hItem)!=LM_TREEID_FILTERITEM&&m_wndTreeCtrl.GetItemData(hItem)!=LM_TREEID_FILTERITEM_REF) )
		return;
...
//Tree选择改变时
void CFtrsMgrDlg::OnSelChanged(NMHDR* pNMHDR, LRESULT* pResult) 
{
	m_dCurSelItem = 0;
...
void CFtrsMgrDlg::OnRclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	*pResult = 0;
...
void CFtrsMgrDlg::OnBeginlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CFtrsMgrDlg::OnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
	TV_DISPINFO* pTVDispInfo = (TV_DISPINFO*)pNMHDR;
...
BOOL CFtrsMgrDlg::PreTranslateMessage(MSG* pMsg) 
{
...
BOOL CFtrsMgrDlg::CanModifyCheckState()
{
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
void CFtrsMgrDlg::ModifyCheckState(CLVLPropItem **pItems, int num, CLVLPropColumn* pCol, int col, _variant_t value)
{
...
void CFtrsMgrDlg::ModifyFtrsAttri(CLVLPropItem *pItem, CLVLPropColumn* Pc)
{
		...
		for (int i=0;i<m_wndPropListCtrl.GetSelectedCount();i++)
		{
			...
			if (pPropitem)
			{
				...
				{	
					...
					else  if (stricmp(Pc->FieldName,StrFromResID(IDS_DBLAYERNAME))==0)
					{
						pItem->dbLayerName = (const char*)(_bstr_t)value;
					}
					else
					{
					...
					CUndoModifyProperties undo(m_pDlgDoc,_T("ModifyFtrProperties"));
...
void CFtrsMgrDlg::ModifyAttri(CLVLPropItem *pItem, CLVLPropColumn* Pc, _variant_t exData)
{
	...
	/*else if (hItem!=NULL && m_wndTreeCtrl.GetItemData(hItem)==LM_TREEID_GROUPS)
	{
		if (m_wndPropListCtrl.GetSelectedCount()<=0)
		{
			return;
		...
		for (int i=0;i<m_wndPropListCtrl.GetSelectedCount();i++)
		{
			...
			if (pPropitem)
			{
				LayerGroup *pGroup = (LayerGroup*)pPropitem->GetData();
				if (pGroup)
				{
					for (int j=0; j<pGroup->arr.GetSize(); j++)
					{
						CFtrLayer *pLayer = pGroup->arr[j];
...
void CFtrsMgrDlg::RButtonDown(UINT nFlags, CPoint point)
{
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
	if(hItem==NULL)
		return;
	...
	if(dw!=LM_TREEID_ALLLEVELS&&dw!=LM_TREEID_ALLLEVELS_REF
		...
		return;
	CMenu obMenu;
	obMenu.LoadMenu(IDR_FTRMANAGER_RB); 
...
void CFtrsMgrDlg::OnAddLayerScheme()
{
	CDlgDataSource *pDS = m_pDlgDoc->GetDlgDataSource();
	if (!pDS) return;
...
void CFtrsMgrDlg::OnDelLayerScheme()
{
	CDlgDataSource *pDS = m_pDlgDoc->GetDlgDataSource();
	if (!pDS) return;
...
void CFtrsMgrDlg::OnSelectFtrs()
{
	...
	if (m_dCurSelItem == LM_TREEID_ALLFEATURES || m_dCurSelItem == LM_TREEID_ALLFEATURES_REF)
	{
		for (i=0; i<arrSelItems.GetSize(); i++)
		{
			...
			{
				m_pDlgDoc->SelectObj(FTR_HANDLE(ftrs->ftrs[j]));
			}
		}
	}
	else
	{
		for (i=0; i<arrSelItems.GetSize(); i++)
		{
			...
			for (int j=0; j<pLayer->GetObjectCount(); j++)
			{
				m_pDlgDoc->SelectObj(FTR_HANDLE(pLayer->GetObject(j)));
			}
...
//取消选择地物
void CFtrsMgrDlg::OnDeSelectFtrs()
{
	CArray<DWORD_PTR, DWORD_PTR> arrSelItems;
	int nCount = m_wndPropListCtrl.GetSelectedCount();
	for (int i=0;i<nCount;i++)
...
int CFtrsMgrDlg::GetCol(CString field)
{
	int nfield = m_wndPropListCtrl.GetColumnCount();
	for( int j=0; j<nfield; j++)
	{
...
void CFtrsMgrDlg::UpdateOrderValue()
{
	int col = GetCol(FIELDNAME_LAYERDISPLAYORDER);
...
void CFtrsMgrDlg::UpdateColorValue()
{
...
BOOL CFtrsMgrDlg::IsFtrscolorByLayer(LONG_PTR dWord)
{
...
BOOL CFtrsMgrDlg::LoadUniqueXAttributesDefine()
{
	m_arrXAttibutes.RemoveAll();
...
//加载扩展属性
BOOL CFtrsMgrDlg::LoadXAttributes(CDlgDataSource *pDS)
{
	if(m_arrMemXAttr.GetCount()>0)
		return TRUE;
...
//加载扩展属性
void CFtrsMgrDlg::OnLoadXAttributes()
{
...
void CFtrsMgrDlg::UpdateStatusString()
{
	int nCount = m_wndPropListCtrl.GetSelectedCount();
	...
	for( i=0; i<nCount; i++)
	{
		...
		nSelFtrsCount += pItem->ftrs.GetSize();
	}
	...
	for (i=0;i<nLayCount;i++)
	{
		...
		if(!pLayer)continue;
		nFtrsCount += pLayer->GetValidObjsCount();
	}

	CString strMsg;
	strMsg.Format(IDS_FTRMAN_STATUS,nFtrsCount,nSelFtrsCount);
