//ºº×Ö×ªÆ´ÒôµÄvc++³ÌÐòÔ´´úÂë/MyChiToLetter.cpp
#include "StdAfx.h"
...
static void ConvertCharToWstring(const char* from, wstring &to, UINT codepage)
{
	to = L"";
...
static void ConvertWcharToString(const wchar_t* from, string &to, 
...
int ConvertCharToTwoChars(LPCTSTR text, char* buf)
{
	wstring to;
	ConvertCharToWstring(text,to,CP_ACP);
	int nChar = to.size();
	char *p = buf;
	wchar_t ch1[2] = {0};
	char    ch2[10] = {0};
	for(int i=0; i<nChar; i++)
	{
		string to2;
		ch1[0] = to[i];
		ConvertWcharToString(ch1,to2,CP_ACP,NULL,0);
		strcpy(ch2,to2.c_str());
		p[0] = ch2[0];
		p[1] = ch2[1];
		p += 2;
	}
...
void CMyChiToLetter::SetFlags(BOOL bSimple, BOOL bFirstBig, BOOL bAllBiG, BOOL bLetterEnd)
{
    m_blnSimple=bSimple;
    m_blnFirstBig=bFirstBig;
    m_blnAllBiG=bAllBiG;
    m_LetterEnd=bLetterEnd;
}

CString CMyChiToLetter::GetLetter(LPCTSTR text)
{
	CString strText = text;

	int nLen = strText.GetLength()*2+10;
	char *buf = new char[nLen];
	memset(buf,0,nLen);

	int nWChar = ConvertCharToTwoChars(text,buf);

    TBYTE ucHigh, ucLow;
    int  nCode,j;
    CString strValue="";
    for (int i=0; i<nWChar; i++)
    {
        if ( (TBYTE)buf[i+i+1]==0 )
        {
            strValue=strValue+_T(buf[i+i]);
            if(m_LetterEnd) strValue=strValue+_T('\'');
            continue;
        }

        ucHigh = (TBYTE)buf[i+i];
        ucLow  = (TBYTE)buf[i+i+1];
        if ( ucHigh < 0xa1 || ucLow < 0xa1)
            continue;
        else
            nCode = (ucHigh - 0xa0) * 100 + ucLow - 0xa0;

        CString strRes=_T(FindLetter(nCode));
        if(m_blnSimple&&strRes.GetLength()>0)
        {
...
CString CMyChiToLetter::FindLetter(int nCode)
{
    CString strValue;
    switch(nCode)
    {
