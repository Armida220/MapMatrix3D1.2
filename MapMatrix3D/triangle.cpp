...
//#define printf(sz)              OutputDebugStr(sz)
...
enum locateresult {INTRIANGLE, ONEDGE, ONVERTEX, OUTSIDE, ERROR_LOCATE};
...
enum finddirectionresult {WITHIN, LEFTCOLLINEAR, RIGHTCOLLINEAR, ERROR_DIR};
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ( (LONG_PTR)ptr & (LONG_PTR) 3);         \
  (otri).tri = (triangle *)                                                   \
                  (  (LONG_PTR)ptr ^ (LONG_PTR) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ( (LONG_PTR)(otri).tri | (LONG_PTR)(otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((LONG_PTR)(otri).tri[6] | (unsigned long) 2l)
...
#define infected(otri)                                                        \
  (((LONG_PTR)(otri).tri[6] & (unsigned long) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((LONG_PTR)(sptr) & (LONG_PTR)1);      \
  (osub).ss = (subseg *)                                                      \
              ( (LONG_PTR)(sptr) & ~ (LONG_PTR) 3)
...
#define sencode(osub)                                                         \
  (subseg) ((LONG_PTR)(osub).ss | (LONG_PTR) (osub).ssorient)
...
#define vertex2tri(vx) (triangle)vx
...

{
  ...
  alignptr = (LONG_PTR)(pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
	  (alignptr + (LONG_PTR)pool->alignbytes -
	  (alignptr % (LONG_PTR)pool->alignbytes));
...

{
  ...
  alignptr = (LONG_PTR)(pool->pathblock + 1);
...

{
  ...
  alignptr = (LONG_PTR)m->dummytribase;
  m->dummytri = (triangle *)
	  (alignptr + (LONG_PTR)m->triangles.alignbytes -
	  (alignptr % (LONG_PTR)m->triangles.alignbytes));
  ...
  if (b->usesegments) {
	...
	alignptr = (LONG_PTR)m->dummysubbase;
    m->dummysub = (subseg *)
		(alignptr + (LONG_PTR)m->subsegs.alignbytes -
		(alignptr % (LONG_PTR)m->subsegs.alignbytes));
...

{
  ...
  alignptr = (LONG_PTR)(getblock + 1);
  foundvertex = (char *)(alignptr + (LONG_PTR)m->vertices.alignbytes -
	  (alignptr % (LONG_PTR)m->vertices.alignbytes));
                         ...
                         vertex searchpoint, struct otri *searchtri)
{
  ...
  while (totalsamplesleft > 0) {
	...
	alignptr = (LONG_PTR)(sampleblock + 1);
    firsttri = (char *) (alignptr +
		(LONG_PTR)m->triangles.alignbytes -
                         (alignptr %
						 (LONG_PTR)m->triangles.alignbytes));
                                       ...
                                       vertex searchpoint)
{
  ...
  vertex startvertex;
  vertex leftvertex, rightvertex, checkvertex;
  ...
  while (leftflag) {
	...
	if( checkvertex==NULL )
		checkvertex = leftvertex;
	else if( checkvertex==leftvertex )
		return ERROR_DIR;
  ...
  while (rightflag) {
	...
	if( checkvertex==NULL )
		checkvertex = rightvertex;
	else if( checkvertex==rightvertex )
		return ERROR_DIR;
...
void markhull(struct mesh *m, struct behavior *b)
{
  ...
  do {
	...
	if( check_tri==NULL )
		check_tri = hulltri.tri;
	else if( check_tri==hulltri.tri )
		break;
...

{
  ...
  *vpointmarkerlist = (int *) NULL;
  ...
  while (triangleloop.tri != (triangle *) NULL) 
  {
	...
	if( torg!=NULL && tdest!=NULL && tapex!=NULL )
	{
