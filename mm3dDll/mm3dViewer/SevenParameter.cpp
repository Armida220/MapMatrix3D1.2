...
void CSevenParameter::CalculateRelativeToAbsolute(CoordDouble3D X_r, CoordDouble3D& X_a)
{
	...
	double fwk[3] = {0};
	transMat::RtoAngle(R_, fwk[0], fwk[1], fwk[2]);
...
void CSevenParameter::CalculateAbsoluteToRelative(CoordDouble3D X_a, CoordDouble3D& X_r)
{
	double xa[3];
	xa[0] = X_a.x;
	xa[1] = X_a.y;
	...
	double pt[3] = { 0 };
	double iR[9];
	TransposeMatrix(R_, 3, 3, iR);
	MatrixMuti1D(iR, 3, 3, xa1, 3, 1, pt);
